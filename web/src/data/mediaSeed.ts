...
import type { AudioResult, ImageResult, TaskEnvelope, VideoResult } from "../lib/media";

// 占位演示数据：图片/音频/视频三个示例任务，素材是 samples/ 里原有的通用测试文件
// （web/public/demo/ 下的静态拷贝，不经过 bff 处理），结构化结果内容也是占位性质的通用样例
// （跟 bff mock 模式的内置样例同一套文案），不是黑龙江真实素材。等拿到真实黑龙江多模态素材后，
// 应该把这里换成"真的跑一遍 bff"产出的结果，而不是继续手写占位内容。

function envelope<R>(id: string, kind: MediaTask["kind"], filename: string, mediaUrl: string, result: R): TaskEnvelope<R> {
  return {
    task_id: id,
    type: kind,
...
const imageResult: ImageResult = {
  layout: "vat_invoice",
  fields: [
    { key: "invoice_code", label: "发票代码", value: "144032509110", confidence: 0.99 },
    { key: "invoice_no", label: "发票号码", value: "25000037", confidence: 0.98 },
    { key: "invoice_date", label: "开票日期", value: "2026-09-01", confidence: 0.97 },
...
export function createMediaSeed(): MediaTask[] {
  const now = new Date().toISOString();
  return [
    {
      id: "seed-image-invoice",
      kind: "image",
      name: "发票识别示例",
      filename: "invoice.jpg",
      mediaUrl: "/demo/invoice.jpg",
      createdAt: now,
      updatedAt: now,
      status: "succeeded",
      envelope: envelope("seed-image-invoice", "image", "invoice.jpg", "/demo/invoice.jpg", imageResult),
    },
    {
      id: "seed-audio-meeting",
      kind: "audio",
      name: "会议转写示例",
      filename: "meeting.wav",
      mediaUrl: "/demo/meeting.wav",
      createdAt: now,
      updatedAt: now,
      status: "succeeded",
      envelope: envelope("seed-audio-meeting", "audio", "meeting.wav", "/demo/meeting.wav", audioResult),
    },
    {
      id: "seed-video-demo",
      kind: "video",
      name: "产品评测视频示例",
      filename: "demo.mp4",
      mediaUrl: "/demo/demo.mp4",
      createdAt: now,
      updatedAt: now,
      status: "succeeded",
      envelope: envelope("seed-video-demo", "video", "demo.mp4", "/demo/demo.mp4", videoResult),
    },
  ];
}
