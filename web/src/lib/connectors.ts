...
export function listConnectors(): Promise<Connector[]> {
  return fetch("/api/connectors", { headers: authHeaders() }).then(jsonOrThrow<Connector[]>);
}

export function testConnector(kind: ConnectorKind, config: ConnectorConfig, secret: string): Promise<TestResult> {
  return fetch("/api/connectors/test", { method: "POST", headers: jsonHeaders(), body: JSON.stringify({ kind, config, secret }) }).then(
...
export function createConnector(kind: ConnectorKind, name: string, config: ConnectorConfig, secret: string): Promise<Connector> {
  return fetch("/api/connectors", { method: "POST", headers: jsonHeaders(), body: JSON.stringify({ kind, name, config, secret }) }).then(
    jsonOrThrow<Connector>,
  );
}

export function deleteConnector(id: string): Promise<void> {
  return fetch(`/api/connectors/${id}`, { method: "DELETE", headers: authHeaders() }).then((res) => {
    if (!res.ok) throw new Error(`删除失败 (${res.status})`);
  });
}

export function listTables(connectorId: string): Promise<string[]> {
  return fetch(`/api/connectors/${connectorId}/tables`, { headers: authHeaders() }).then(jsonOrThrow<string[]>);
}

export function extractSqlRows(connectorId: string, table: string, limit: number): Promise<ExtractResult> {
  return fetch(`/api/connectors/${connectorId}/extract`, {
    method: "POST",
    headers: jsonHeaders(),
